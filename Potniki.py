...
def razdalje(kraj, kraji):
    pari = set()

    for ime in kraji:
        if ime[0] == kraj:
            ra1, raz1 = ime[1], ime[2]

    for ime in kraji:
        if ime[0] != kraj:
            ra2, raz2 = ime[1], ime[2]
            razdalja_med_kraji = (abs(ra1 - ra2) * abs(ra1 - ra2)) + (abs(raz1 - raz2) * abs(raz1 - raz2))

            pari.add((razdalja_med_kraji, ime[0]))
    return pari


def sosedi(n, kraj, kraji):
    so_mn = set()
    sosednji = sorted(list(razdalje(kraj, kraji)))
    co = 0

    for sosed in sosednji:
        if co >= n:
...
def sosedi(n, kraj, kraji):
    return {ime for _, ime in sorted(razdalje(kraj, kraji)) [:n]}


def povezave(n, kraji):
    slovar = {}

    for a in kraji:
        slovar[a[0]] = sosedi(n, a[0], kraji)

        if a[0] not in slovar:
            slovar[a[0]] = None
...
def pvezave(n, kraji):
    return {kraj:sosedi(n, kraj, kraji) for kraj, _, _ in kraji}


def dvosmerno(d):
    sl_sk = deepcopy(d)

    for pov1, pov2 in d.items():
        for povezav in sorted(pov2):
            if povezav in pov2:
                sl_sk[povezav].add(pov1)
...
def potnik(zacetek, korakov, povezave):
    ...
    for korak in range(korakov):
        naslednji = izberi(povezave[kje_sem], prepovedan)
